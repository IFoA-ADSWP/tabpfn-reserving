...
| **A4 — distributional classification** | Predict `P(development exceeds threshold)` for a ladder of thresholds; rebuild a distribution from the survival function | **Corrected 2026-09-19** (`results/remodel/FINDINGS.md` §1.6): the original rationale — "this project's own house finding that classification is the model's strongest axis and regression is not" — has **no source and is contradicted for 3.5**, which is trained jointly on both tasks and leads a distributional benchmark. What *is* documented is the mechanism: regression **is** classification over a bucket grid that is fixed on the pretraining prior and merely rescaled to the target's mean and sd, so a ladder we place ourselves puts resolution where the fixed grid is coarse — and lets the tail be checked level by level |
